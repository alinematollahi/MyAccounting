...
import RemoveExpenseCategory from './removeExpenseCategory';

const EDIT_EXPENSE_CATEGORY = gql`
mutation($id: ID , $newCategoryName: String ,$oldCategoryName: String) {
    editUser(_id: $id , userEdit:{
        oldExpenseCategorys:{categoryName:$oldCategoryName}
        newExpenseCategorys:{categoryName:$newCategoryName}
    }){
        expenseCategorys{             
            categoryName
        }
...
export default function ExpenseCategoryTable() {
    ...
    const [editUser, { data, loading, error }] = useMutation(EDIT_EXPENSE_CATEGORY);
    ...
    React.useEffect(() => {
        if (data) {
            ...
            dispatch(getExpenseCategory(data.editUser.expenseCategorys));
